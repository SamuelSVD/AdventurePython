...
class TextureLoader:
	...
	def loadEnvironmentTexture(self):
		...
		for i in range(10):
			floor = pygame.Surface((32,64),pygame.SRCALPHA)
			floor.blit(image, (0,0), (32*i, 0, 32,64))
			self.environment_texture.append(floor)
...
class TextWindow(Component):
	...
	def draw(self, screen):
		...
		for i in range(len(self.lines)):
			...
			self.window_surface.blit(self.font.render(self.lines[i],1,(220,220,220)),(5,self.height+ 2 - (self.line_height) * (len(self.lines) - i) + self.line_height * self.scroll))
		...
		screen.blit(self.font.render(str(self.focus), 1, (255,255,255)), (10,10))
...
class Gameplay(Component):
	def __init__(self,character = None):
		...
		self.text_window = TextWindow((20,20),(20,550),600,pygame.font.SysFont('calibri', 16))
	...
	def draw(self, screen):
		if not self.character:
			...
			return
		#else:
			#print 'YAY! We have a character'
		offset = screen.get_size()
		offset = (offset[0]/2,offset[1]/2)
		for entity in self.entities:
			if entity is self.character:
				#print 'drawing character'
				image = entity.getShadow()
				size = image.get_size()
				screen.blit(image,(-size[0]*0.5+offset[0],-size[1]*0.5+offset[1]))
				image = entity.getImage()
				size = image.get_size()
				screen.blit(image,(-size[0]/2+offset[0],-size[1]*0.7+offset[1]))
				#pygame.draw.rect(screen, (255,255,255),(offset[0], offset[1],2,2))
				continue
			image = entity.getShadow()
			size = image.get_size()
			relative_position = Vector(entity.position.x,entity.position.y)
			relative_position.add(self.character.position.scale(-1))
	...
	def update(self, delta = None):
		...
		if self.total_time > self.nextPoll:
			...
			self.poll()
		for entity in self.entities:
			#print self.entities.index(entity), entity.get(), delta
			entity.update(delta)
	def handleKey(self, key_event):
	...
	def handleMessageTCP(self, message):
		...
		if message_type == 'c': #This client's character
			if self.character:
				...
				self.character = Character(message[0], Vector(float(message[1]), float(message[2])), int(message[3]), int(message[4]), int(message[5]), message[6], None, None, None)
...
#			self.entities.append(Entity(message[0], Vector(float(message[1]),float(message[2])), int(message[3]), int(message[4]), int(message[5]),charAnim()))
#		elif message_type == 'R':
#			message = message[1:].split()
#			self.character = Entity(message[0], Vector(float(message[1]),float(message[2])), int(message[3]), int(message[4]), int(message[5]),charAnim())
#			self.entities.append(self.character)
#		elif message_type == 'D':
#			message = message[1:]
#			if self.character and self.character.id:
#				if self.character.id == message:
#					self.terminate = True
#			for entity in self.entities:
...
#		print 'The location is:',repr(self.location),temp[0], temp[1]
	...
	def buildTexture(self):
		...
		for i in range(len(self.environment)):
			...
			if count == 0:
				print 'continued'
				row = pygame.surface.Surface((10,10), pygame.SRCALPHA)
				row.set_alpha(0)
				row.fill((255,200,100))
